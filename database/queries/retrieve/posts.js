...
export const selectPosts = async () => {
    const sql = `SELECT p.link, p.commenter, p.tittle, p.description, p.image, u.name, u.picture, u.id FROM posts p JOIN users u ON u.id = p."idUser" ORDER BY p."createDate" DESC`
